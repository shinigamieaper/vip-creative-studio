...
export interface ResourcesFeaturedHeroProps extends SectionProps {
  resource: Resource;
}

const ResourcesFeaturedHero: React.FC<ResourcesFeaturedHeroProps> = ({
  className,
  resource,
  ...props
}) => {
  const topicLabel =
    getPrimaryTopicLabelForResource(resource) ?? resource.category;
      ...
      <div className="w-full max-w-[1140px] mx-auto px-6 md:px-10">
        ...
        <article className="relative overflow-hidden rounded-[32px] bg-card shadow-[0_16px_40px_rgba(0,0,0,0.18)]">
          <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8 lg:p-10 flex flex-col gap-4 text-white">
                <div className="inline-flex items-center gap-2 text-xs font-medium text-white/80">
                  ...
                  <span className="text-white/70">
                    {resource.publishedAt} · {resource.readTime} min read
                  </span>
                </div>
            ...
            {/* Right column: author + meta (desktop) */}
            <div className="hidden lg:flex flex-col justify-between p-8">
              ...
              <div className="mt-8 flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-primary">{resource.author.name}</p>
                  <p className="text-xs text-primary/60">{resource.author.role}</p>
                </div>
              </div>
            </div>
          </div>
...
export default ResourcesFeaturedHero;
