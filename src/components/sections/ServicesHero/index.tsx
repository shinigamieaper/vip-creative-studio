...
export interface ServicesHeroProps extends React.ComponentPropsWithoutRef<'section'> {
  titlePrefix?: string;
  flipWords?: string[];
  description?: string;
  showCtas?: boolean;
...
const ServicesHero: React.FC<ServicesHeroProps> = ({
  ...
  const resolvedTitlePrefix =
    titlePrefix ?? "More Than a Vendor. We're Your Fractional Partner for";
  const resolvedFlipWords =
    flipWords && flipWords.length > 0 ? flipWords : defaultFlipWords;
  ...
  const handleSecondaryClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    if (!resolvedSecondaryHref.startsWith('#')) {
      return;
    }

    e.preventDefault();
    const target = document.querySelector(resolvedSecondaryHref);
                  ...
                  <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-6">
                    ...
                    {showSecondary && (
                      <PrimaryCtaButton
                        ...
                        withArrow
                      >
                        <a href={resolvedSecondaryHref} onClick={handleSecondaryClick}>{resolvedSecondaryLabel}</a>
                      </PrimaryCtaButton>
                    )}
                  </div>
                )}
